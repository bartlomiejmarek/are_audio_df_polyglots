...
class RawGAT_ST(nn.Module):
    def __init__(self, d_args: dict, device: str):
        ...
        self.proj = nn.Linear(16, d_args["nb_classes"])
    ...
    def forward(self, x, Freq_aug=False):
        ...
        x_gat1 = self.GAT_layer1(
            x_max.transpose(1, 2)
        )  # (#bs,#node(F),feat_dim(C)) --> [#bs, 23, 32]

        x_pool1 = self.pool1(x_gat1)
        out1 = self.proj1(x_pool1.transpose(1, 3))
        out1 = out1.view(
            out1.shape[0], out1.shape[1], out1.shape[3]
        )  # (#bs,feat_dim,#node) --> [#bs, 32, 12]
        ...
        x_max2, _ = torch.max(
            torch.abs(e2), dim=2
        )  # max along frequency  #[#bs, C(64), T(29)]

        x_gat2 = self.GAT_layer2(
            x_max2.transpose(1, 2)
        )  # (#bs,#node(T),feat_dim(C)) --> #[#bs, 29, 32]

        x_pool2 = self.pool2(x_gat2)
        out2 = self.proj2(x_pool2.transpose(1, 3))
        out2 = out2.view(
            out2.shape[0], out2.shape[1], out2.shape[3]
        )  # (#bs,feat_dim,#node)  #[#bs, 32, 12]
        ...
        x_gat3 = self.GAT_layer3(
            out_gat.transpose(1, 2)
        )  # (#bs,#node,feat_out_dim) --> #[#bs, 12, 16]

        x_pool3 = self.pool3(x_gat3)

        out_proj = self.proj(x_pool3).flatten(1)  # (#bs,#nodes) --> [#bs, 7]

        output = self.proj_node(
            out_proj
        )  # (#bs, output node(no. of classes)) ---> [#bs,1]
