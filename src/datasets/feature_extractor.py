...
def create_mfcc_transform(**kwargs) -> torchaudio.transforms.MFCC:
    ...
    return torchaudio.transforms.MFCC(
        sample_rate=kwargs.get("sample_rate", configuration.audio_config.sample_rate),
        n_mfcc=kwargs.get("n_mfcc", configuration.audio_config.n_mfcc),
        melkwargs={
            "n_fft": kwargs.get("n_fft", configuration.audio_config.n_fft),
            "win_length": kwargs.get("win_length", configuration.audio_config.win_length),
            "hop_length": kwargs.get("hop_length", configuration.audio_config.hop_length),
        },
...
def create_lfcc_transform(**kwargs) -> torchaudio.transforms.LFCC:
    ...
    return torchaudio.transforms.LFCC(
        sample_rate=kwargs.get("sample_rate", configuration.audio_config.sample_rate),
        n_lfcc=kwargs.get("n_lfcc", configuration.audio_config.n_lfcc),
        speckwargs={
            "n_fft": kwargs.get("n_fft", configuration.audio_config.n_fft),
            "win_length": kwargs.get("win_length", configuration.audio_config.win_length),
            "hop_length": kwargs.get("hop_length", configuration.audio_config.hop_length),
...
def create_mel_scale_transform(**kwargs) -> torchaudio.transforms.MelScale:
    ...
    return torchaudio.transforms.MelScale(
        n_mels=kwargs.get("n_mels", configuration.audio_config.n_mels),
        n_stft=kwargs.get("n_stft", configuration.audio_config.n_stft),
        sample_rate=kwargs.get("sample_rate", configuration.audio_config.sample_rate),
    ).to(configuration.device)
...
def create_delta_transform(**kwargs) -> torchaudio.transforms.ComputeDeltas:
    ...
    return torchaudio.transforms.ComputeDeltas(
        win_length=kwargs.get("delta_win_length", configuration.audio_config.delta_win_length),
        mode="replicate",
    )
...
def prepare_feature(
    ...
    return x[:, :, :, :kwargs.get("max_frames", configuration.audio_config.max_frames)]
